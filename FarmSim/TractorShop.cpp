...
TractorShop::TractorShop(Vec3 pos, ShopParams *shopParams)
{
	...
	{
		m_items.push_back(new TractorShopItem());
		m_items[i]->fname = shopParams->shopItems[i]->fname;
		m_items[i]->typeName = shopParams->shopItems[i]->typeName;
		m_items[i]->price = shopParams->shopItems[i]->price;
...
bool TractorShop::checkIfParkingIsFree(int x, int y, float interval)
{
...
DeviceShopType TractorShop::getShopType()
{
	return DST_TRACTOR;
}